  ...
  return (
    <div className="bg-onyx text-white font-sans min-h-screen selection:bg-gold-accent selection:text-black overflow-hidden">
      ...
      <nav ref={navRef} className="fixed top-0 w-full px-6 md:px-16 py-6 flex justify-between items-center z-50 backdrop-blur-2xl bg-onyx/80 border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.5)] opacity-0">
        ...
        {/* Mobile Menu Overlay */}
        <div className={`fixed inset-0 bg-onyx z-40 transition-all duration-500 flex flex-col justify-start items-center gap-12 pt-40 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
          {['PROCESS', 'PORTFOLIO', 'CONTACT'].map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase()}`}
              className="text-3xl font-bold tracking-[4px] text-white hover:text-gold-accent transition-colors"
      ...
      <div className="container mx-auto px-6 md:px-12 max-w-7xl relative">
        ...
        <section className="pt-48 pb-20 relative" ref={heroRef}>
          {/* Agent NEO: 3D Layer - Fixed for Mobile (Centered, Z-Index fixed, Full Opacity) */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[50vh] md:right-0 md:left-auto md:translate-x-0 md:w-1/2 md:h-full z-0 md:z-10 pointer-events-none opacity-100">
            <Scene3D />
          </div>
